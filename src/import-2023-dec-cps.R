...
# on food stamps
ipums_extract_2023_dec <- define_extract_micro(
  description = "Food insecurity: 2023 Dec CPS",
  collection = "cps",
  samples = c(
    "cps2023_12s" # Dec 2023 CPS
  ),
  variables = c(
    "COUNTY", "AGE", "SCHLCOLL", 
    "FSSTATUSD",
...
submitted <- submit_extract(ipums_extract_2023_dec)
...
dbWriteTable(con, "ipums_2023_dec", ipums_tb, overwrite = TRUE)
